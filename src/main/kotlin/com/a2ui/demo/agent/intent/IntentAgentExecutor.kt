        ...
        return AIAgent(
            ...
            strategy = strategy<A2AMessage, Unit>("intent-agent") {
                ...
                val nodeForwardToGenerator by node<String, Unit> { intentJson ->
                    ...
                    // Extraire la base URL (sans le path) pour l'agent card resolver
                    // generatorUrl = "http://localhost:9999/generator" → baseUrl = "http://localhost:9999"
                    ...
                    try {
                        ...
                        val communicationEvent = response.data

                        val responseText = when (communicationEvent) {
                            is Message -> {
                                communicationEvent.parts
                                    .filterIsInstance<TextPart>()
                                    .joinToString("") { it.text }
                            }
                            is Task -> {
                                communicationEvent.status.message?.parts
                                    ?.filterIsInstance<TextPart>()
                                    ?.joinToString("") { it.text }
                                    ?: "Erreur: pas de contenu texte dans la réponse Task"
                            }
                        }

                        withA2AAgentServer {
                            updateTaskStatus(responseText, TaskState.Completed, final = true)
                        }
                    } catch (e: Exception) {
                        withA2AAgentServer {
                            updateTaskStatus(
